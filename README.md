...
[![Code Coverage](https://img.shields.io/badge/Coverage-100%25-brightgreen.svg)](https://github.com/alexandrainst/ragger/tree/main/tests)
...
In the `src` directory there are two subdirectories, `ragger`
and `scripts`. This is a brief explanation of the differences between the two.

### Modules
All Python files in the `ragger` directory are _modules_
...
```
from ragger import some_function
```
...
`alexandrainst/ragger` then simply press the `<> Code` button
